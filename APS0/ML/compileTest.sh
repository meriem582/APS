...
if [ "$#" -gt 0 ]; then
    ...
    for file in "$@"; do
        if [ -f "$file" ]; then
            ...
            output_file="$output_dir/ast_${filename%.*}.txt"  # Stocker dans APS_resultat/
    ...
    if [ ! -d "Samples" ]; then
        echo "⚠️  Le dossier 'Samples/' n'existe pas. Créez-le et ajoutez des fichiers .pstl."
        exit 1
    fi

    echo "🔍 Recherche des fichiers .pstl dans le dossier Samples/..."
    found=0
    ...
    if [ "$found" -eq 0 ]; then
        echo "⚠️  Aucun fichier .pstl trouvé dans Samples/"
    fi
fi
