...
/*Block*/
is_type_block(G, block(CS), T) :- is_type_cmds(G, CS, T).
...
is_type_cmds(G, [cmds(D)|CS], T) :- is_type_def(G,D,G1), is_type_cmds(G1,CS,T).


/* END */
/* passer S au jugement de typage is_type_stat */
is_type_cmds(G, X, T) :- is_type_stat(G,X,T).
...
/* STAT1 */
is_type_cmds(G,[S|CS],T) :- is_type_stat(G,S,union(T,void)), is_type_cmds(G,CS,T).

is_type_cmds(G,[S], T) :- is_type_stat(G,S,T), T\=void.
...
/* FUNP */
is_type_def(G,funBlock(X, T, ARGS, BK), GI) :- rec_type_args(ARGS, TI), append(ARGS, G, G1), G2 = [(X, arrow(TI, T))|G1], is_type_block(G2, BK, T), GI=[(X, arrow(TI, T))|G].

/* FUNRECP */
is_type_def(G,funRecBlock(X, T, ARGS, BK), GI) :- rec_type_args(ARGS, TI), append(ARGS, G, G1), G2 = [(X, arrow(TI, T))|G1], is_type_block(G2, BK, T), GI=[(X, arrow(TI, T))|G].
...
/*SET*/
is_type_stat(G,set(LVALUE,E),void) :- is_type_lvalue(G,LVALUE,T), is_type_expr(G,E,T).

/*IF0*/
is_type_stat(G,ifStat(E,BK1,BK2),T) :- is_type_expr(G,E,bool), is_type_block(G,BK1,T), is_type_block(G,BK2,T).

/*IF1*/
is_type_stat(G,ifStat(E,BK1,BK2),union(T,void)) :- is_type_expr(G,E,bool), is_type_block(G,BK1,void), is_type_block(G,BK2,T).

/*IF2*/
is_type_stat(G,ifStat(E,BK1,BK2),union(T,void)) :- is_type_expr(G,E,bool), is_type_block(G,BK1,T), is_type_block(G,BK2,void).


/*WHILE*/
is_type_stat(G,while(E,BK),union(T, void)) :- is_type_expr(G,E,bool), is_type_block(G,BK,T).
...
is_type_lvalue(G,lvalNth(E1,E2),T) :- is_type_expr(G,E1,vec(T)), is_type_expr(G,E2,int).
...
is_type_expr(G,app(E, EI), T) :- is_type_expr(G,E,arrow(TI,T)), check_args(G,EI,TI).
...
/* ALLOC */
is_type_expr(G, alloc(E), vec(_)) :- is_type_expr(G,E,int).

/* LEN */
is_type_expr(G, len(E), int) :- is_type_expr(G,E,vec(_)).

/*NTH */
is_type_expr(G, nth(E1,E2), T) :- is_type_expr(G,E1,vec(T)), is_type_expr(G,E2,int).

/* VSET */
is_type_expr(G, vset(E1,E2,E3),vec(T)) :- is_type_expr(G,E1,vec(T)), is_type_expr(G,E2,int), is_type_expr(G,E3,T).
